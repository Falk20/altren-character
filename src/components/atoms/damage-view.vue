<template>
  <div class="d-flex flex-wrap text-body-1">
    ...
    <div
      v-if="
        Object.keys(flatDamage.dices).length === 0 && !flatDamage.modificator
      "
    >
      0
    </div>

    <div v-for="(value, key, index) in flatDamage.dices" :key="'dice' + key">
      {{ index > 0 ? "+" : "" }}{{ value > 1 ? value + "*" : ""
      }}<v-icon color="green" :icon="key" />
    </div>
